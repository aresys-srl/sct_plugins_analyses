...
maps_directory = ""                            # path to the tec map directory
analysis_center = "cor"                        # analysis center (maps provider)
tec_incidence_angle_method = "ground_converted" # TEC incidence angle method
```

??? danger "Validation"

    ``analysis_center`` and ``tec_incidence_angle_method`` map to internal *enum classes* and are validated
    by the schema to match these valid values.
    Here are the possible values:  
    ==analysis_center==: __``cor``__, __``cod``__, __``esa``__, __``esr``__  
    ==tec_incidence_angle_method==: __``ipp``__, __``ground``__, __``ground_converted``__
...
maps_directory = ""                             # path to the maps directory
map_grid_resolution = "fine"                    # maps grid resolution
```

??? danger "Validation"

    ``map_grid_resolution`` maps to an internal *enum class* and is validated by the schema to match the valid values.  
    Here are the possible values:
    ==map_grid_resolution==: __``fine``__, __``coarse``__  
