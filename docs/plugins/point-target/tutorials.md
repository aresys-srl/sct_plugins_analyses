...
- **External Orbit File**: provides precise orbit state vectors used for Doppler centroid and squint angle computation
- **External Corrections Product**: provides pre-computed corrections (e.g. ETAD for Sentinel-1) to refine the Absolute Localization Error
...
maps_directory = "path/to/ionospheric/maps/directory"
analysis_center = "cor"

[point_target_analysis.corrections.troposphere]
maps_directory = "path/to/tropospheric/maps/directory"
...
```bash
sct --config path_to_config_toml point_target ...
```
...
!!! tip "Maps Download"
    ...
    > :lucide-circle-chevron-right: Refer to the [CLI utility commands](https://opensource.aresys.it/sct/documentation/CLI/cli_utilities/) for more details.
