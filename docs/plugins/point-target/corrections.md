...
# Localization Errors Corrections
...
- ==**External ALE Range and Azimuth corrections**==: if an external ALE corrections product is provided (i.e. ETAD), range
and azimuth corrections can be applied
...
provided by Professor V. Dehant [^1]. This correction *does not take into account* ocean loading, atmospheric loading, or
deformation due to polar motion.

!!! abstract "Fortran Implementation"
...
Plate tectonics displacements are computed using an internal model based on *ITRF2014 plate motion model* [^2]. For more
...
**IONEX** (**ION** onosphere **EX** change) format generated from **NASA GNSS-CDDIS** data and analyzed from specific analysis center [^3].
...
Function 3** (VMF3) model [^4]. Four maps are needed to perform the interpolation at best:
...
<figure markdown="span">
    ![Troposphere Products](../../assets/images/troposphere_data_maps_model.png){ width="750" }
    <figcaption>Scheme of supported troposphere products.</figcaption>
</figure>

These maps must be provided as an input when this correction is enabled via the configuration file.

!!! warning "Maps Format"

    Only  **Operational (OP) GRID** data for VMF3 are supported:

    - 1x1 (fine) grid is the default.
    - 5x5 (coarse) can be provided specifying the change of grid type in the configuration file.
...
-   :lucide-satellite:{ .lg .middle } __Sentinel-1__
...
[^1]: For more information, check the [official GitHub page](https://geodesyworld.github.io/SOFTS/solid.htm) of **Solid** project.

[^2]: Zuheir Altamimi et al., *"ITRF2014 plate motion model"*, Geophysical Journal International, 2017,
[https://doi.org/10.1093/gji/ggx136](https://doi.org/10.1093/gji/ggx136)

[^3]: For more information, check the [official website](https://www.earthdata.nasa.gov/data/space-geodesy-techniques/gnss/atmospheric-products)

[^4]: For more information, check the [CDDIS website](https://cddis.nasa.gov/archive/gnss/products/troposphere/zpd) or
the [VMF Data Server](https://vmf.geo.tuwien.ac.at/)
