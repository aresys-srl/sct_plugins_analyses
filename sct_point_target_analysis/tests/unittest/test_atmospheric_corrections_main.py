...
from sct_point_target_analysis.config import (
    IonosphericCorrectionsConf,
    SCTPointTargetAnalysisCorrectionsConf,
    TroposphericCorrectionsConf,
)
from sct_point_target_analysis.core.atmospheric_corrections_main import (
...
def test_run_compute_atmospheric_delays(mocker):
    mocker.patch(
        "sct_point_target_analysis.core.atmospheric_corrections_core.inverse_geocoding_monostatic",
        return_value=(PreciseDateTime.from_numeric_datetime(2000), None),
    )
    mocker.patch(
        "sct_point_target_analysis.core.atmospheric_corrections_core.ionosphere.compute_delay",
        return_value=0.1,
    )
    mocker.patch(
        "sct_point_target_analysis.core.atmospheric_corrections_core.troposphere.compute_delay",
        return_value=0.2,
    )
