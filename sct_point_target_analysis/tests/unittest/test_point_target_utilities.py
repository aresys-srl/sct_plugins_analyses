...
from sct_point_target_analysis.core.utilities import (
    update_df_with_llh,
...
def test_update_results_with_derived_quantities():
    ...
    assert updated.loc[0, "total_ale_range_correction_[m]"] == 0.4
    assert updated.loc[0, "total_ale_azimuth_correction_[m]"] == 0.5
    assert updated.loc[0, "revised_ale_range_[m]"] == 1.4
    assert updated.loc[0, "revised_ale_azimuth_[m]"] == 3.5
...
def test_update_results_with_theoretical_rcs(monkeypatch):
    ...
    monkeypatch.setattr(
        "sct_point_target_analysis.core.utilities._compute_theoretical_rcs",
        lambda *args, **kwargs: [24.5],
    )
...
def test_update_targets_with_geodynamics_corrections(monkeypatch):
    ...
    monkeypatch.setattr(
        "sct_point_target_analysis.core.utilities.run_compute_geodynamics_corrections",
        lambda *args, **kwargs: np.array([[1.0, 2.0, 3.0]]),
    )
