...
"""Testing SCT Point Target Analysis plugin interface."""
...
from sct_point_target_analysis.interface import ANALYSIS_NAME, PointTargetAnalysisPlugin


@pytest.fixture
def plugin():
    plugins = import_analysis_plugins()
    candidates = [p for p in plugins if p is PointTargetAnalysisPlugin]
...
class TestPluginProtocolCompliance:
    ...
    def test_installed_plugin(self, plugin) -> None:
        ...
        assert plugin is PointTargetAnalysisPlugin
        assert plugin.__name__ == "PointTargetAnalysisPlugin"
    ...
    def test_handler_structure(self, plugin) -> None:
        ...
        assert handler.config is SCTPointTargetAnalysisConfig
