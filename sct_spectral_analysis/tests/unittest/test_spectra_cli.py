...
command = "spectra"
test_configuration = SCTSpectralAnalysisConfig()
...
def test_invalid_product(tmp_path):
    ...
    input_product.mkdir()
    point_targets = tmp_path / "point_targets"
    point_targets.touch()
...
def test_invalid_product_1(tmp_path):
    ...
    test_configuration.to_toml(conf_file)
    command_args = f"--config {conf_file} {command} -p {input_product} -pt {point_targets} -out {output_dir}".split()
    result = cli_runner.invoke(app, command_args)
    assert result.exit_code == 1
