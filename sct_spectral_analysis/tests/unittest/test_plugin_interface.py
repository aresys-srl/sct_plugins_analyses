...
"""Testing SCT Point and Distributed Target Spectral Analysis plugin interface."""
...
from sct_spectral_analysis.interface import ANALYSIS_NAME, SpectralAnalysisPlugin


@pytest.fixture
def plugin():
    plugins = import_analysis_plugins()
    candidates = [p for p in plugins if p is SpectralAnalysisPlugin]
...
class TestPluginProtocolCompliance:
    ...
    def test_installed_plugin(self, plugin) -> None:
        ...
        assert plugin is SpectralAnalysisPlugin
        assert plugin.__name__ == "SpectralAnalysisPlugin"
    ...
    def test_handler_structure(self, plugin) -> None:
        ...
        assert handler.config is SCTSpectralAnalysisConfig
