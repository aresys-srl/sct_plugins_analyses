...
"""Testing SCT Notch Analysis plugin interface."""
...
from sct_notch_analysis.interface import ANALYSIS_NAME, ElevationNotchAnalysisPlugin


@pytest.fixture
def plugin():
    plugins = import_analysis_plugins()
    candidates = [p for p in plugins if p is ElevationNotchAnalysisPlugin]
...
class TestPluginProtocolCompliance:
    ...
    def test_installed_plugin(self, plugin) -> None:
        ...
        assert plugin is ElevationNotchAnalysisPlugin
        assert plugin.__name__ == "ElevationNotchAnalysisPlugin"
    ...
    def test_handler_structure(self, plugin) -> None:
        ...
        assert handler.config is SCTElevationNotchAnalysisConfig
